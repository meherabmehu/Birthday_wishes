...
function startCelebrationCanvas() {
  ...
  function resize() {
    ...
    const total = width < 600 ? 90 : 150;
    petals = Array.from({ length: total }, (_, i) => makePetal(i < total * .78));
  }
  function makePetal(scatter) {
    return { x: Math.random() * width, y: scatter ? Math.random() * height : -30 - Math.random() * height, size: 4 + Math.random() * 10, speed: .7 + Math.random() * 2.25, drift: (Math.random() - .5) * .75, phase: Math.random() * Math.PI * 2, rotation: Math.random() * 6.28, spin: (Math.random() - .5) * .075, kind: Math.random() < .17 ? 'heart' : Math.random() < .18 ? 'star' : 'petal', color: palette[Math.floor(Math.random() * palette.length)] };
  }
  ...
  function draw(p) {
    ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(p.rotation); ctx.globalAlpha = .82;
